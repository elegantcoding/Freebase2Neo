...
import org.neo4j.kernel.impl.transaction.xaframework.LogEntry.Done
...
class Freebase2Neo(inserter: BatchInserter, settings: Settings) {
  ...
  case object CreateDBActor {
    case object Start
  }
  class CreateDBActor extends Actor {
    override def preStart = {
        getIdsPass
        persistIdMap
        createNodes
        val createRelationshipsPass = context.actorOf(Props[CreateRelationshipsPassActor], "relpass")
    }
    def receive = {
      case _ =>
  ...
  def createDb = {
    akka.Main.main(Array(classOf[CreateDBActor].getName))
  }
  ...
  object BatchInserterActor {
    case class RelTriple(subjectString:String, predicateString:String, objectString:String, count:Long)
    case object Done
  }
  class BatchInserterActor extends Actor {
    var relationshipCount = 0l
    val start = System.currentTimeMillis
    var count = 0l
    def receive = {
      case triple:BatchInserterActor.RelTriple => {
        count = triple.count
      ...
      case BatchInserterActor.Done => {
        Utils.displayDone(stage, "create relationships", start, count, "triples", relationshipCount, "relationships")
        sender() ! CreateRelationshipsPassActor.Done
        context.stop(self)
      }
    }
  }

  object CreateRelationshipsPassActor {
    case object Done
  }
  class CreateRelationshipsPassActor extends Actor {
    override def preStart = {
      logger.info("starting create relationships pass...")
      stage += 1
      val relCreator = context.actorOf(Props[BatchInserterActor], "relinserter")
      ...
      rdfIterable.foreach {
        ...
        // if subject is an mid
          if (triple.subjectString.startsWith("<http://rdf.freebase.com/ns/m.")) {
            relCreator ! BatchInserterActor.RelTriple(triple.subjectString, triple.predicateString, triple.objectString, count)
          }
          count = count + 1
      }
      relCreator ! BatchInserterActor.Done
    }
